...
units : pint.UnitRegistry
    The registry for units tracked by ``immlib``. The ``immlib.units`` object is
    a global ``pint``-module unit registry that can be used as a single global
    place for tracking units. Immlib functions that interact with units
